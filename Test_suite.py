...
# Draw text with optional full-row highlight
def draw_highlighted_text(oled, text, x, y, highlight=False):
    ...
    if highlight:
        ...
        oled.text(text, x, y, 0)  # Black text on white highlight
    else:
        oled.text(text, x, y, 1)  # Normal white text


# === Show error screen for failed components ===
...
# Test ultrasonic sensors for 10 seconds
...
# Motor test: move in all directions
...
# IR array sensor test with repeat detection
def test_ir_array(car):
    ...
    while time.time() - start_time < duration:
        ...
        if readings is None or any(r is None for r in readings):
            return show_error("IR Sensor")

        if previous_readings == readings:
...
test_options = [
    ...
    ("5.Color sensor", test_ir_array),  # Placeholder for future sensor
]

# Display scrolling test menu
def show_test_menu(selected):
    car.OLED.fill(0)
    draw_medium_text(car.OLED, "MENU", 40, 0)
    max_visible = 4
    start_index = max(0, selected - max_visible + 1)
...
try:
    while True:
        ...
        # Left button (scroll/select/back)
        if car.is_buttonL_pressed():
            counter = sum(not car.is_buttonL_pressed() for _ in range(20))
            if in_main_menu:
                if counter < 20:
        ...
        # Right button (scroll/select/enter)
        if car.is_buttonR_pressed():
            counter = sum(not car.is_buttonR_pressed() for _ in range(20))
            if in_main_menu:
                if counter < 20:
                    main_menu_selection = (main_menu_selection + 1) % 2
                    show_main_menu(main_menu_selection)
                ...
                if counter < 20:
                    ...
                    _, test_func = test_options[test_menu_selection]
                    test_func(car)
                    show_test_menu(test_menu_selection)
            time.sleep(0.2)
